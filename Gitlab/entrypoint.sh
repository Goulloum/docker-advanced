...
#Get auth token
AUTH=$(curl --request POST --header "PRIVATE-TOKEN: ${PAT_TOKEN}" --data "runner_type=instance_type" \
     "${CI_SERVER_URL}/api/v4/user/runners")

TOKEN=$(echo $AUTH |  grep -o '"token":"[^"]*' | grep -o '[^"]*$')
echo "${TOKEN} is OK"

gitlab-runner register --non-interactive --name="d-in-d" --url="${CI_SERVER_URL}" --token="${TOKEN}"  --executor="docker" --docker-image="docker:latest"
